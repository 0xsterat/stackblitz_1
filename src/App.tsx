...
function App() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`https://society.ton.org/v1/users/${username}/sbts?_start=0&_end=500`)
      if (!response.ok) {
        throw new Error('Failed to fetch data')
      }
      const data = await response.json()
      
      const isOG = data.some((item: any) => item.sbt_collections_id === 1245)
      setResult(isOG ? 'You are an OG champ!' : 'You are not an OG champ.')
    } catch (err) {
